...
void add_new_frames(FrameNode** head)
{
    ...
    newFrame->name = (char*)malloc(FRAME_NAME_SIZE * sizeof(char));

    printf("*** Creating new frame ***\n");
    printf("Please insert frame path:\n");
    myFgets(newFrame->path, PATH_SIZE);

    printf("Please insert frame duration <in milliseconds>:\n");
    scanf_s("%u", &(newFrame->duration));
    getchar();

    printf("Please choose a name for that frame: \n");
    myFgets(newFrame->name, PATH_SIZE);
    

    // checking if the name was already existing
    if (isFrameNameExists(*head, newFrame->name)) 
    {
    ...
    if (!checkImageFileExistence(newFrame->path)) 
    {
    ...
    if (*head == NULL) // set the new node as the head if its empty
    {
        *head = newNode;
        return;
    }
    ...
    FrameNode* curr = *head; 
    while (curr->next != NULL)  // appending the new node
    {
        curr = curr->next;
    }
    curr->next = newNode;
